...
$sql = $cnnPDO->prepare("SELECT numero_c_remitente, nombre_remitente, monto, numero_c_destinatario, nombre_destinatario, DATE_FORMAT(fecha, '%d-%m-%Y %H:%i:%s') AS fecha_formateada
    FROM transferencias 
    WHERE numero_c_remitente = ? OR numero_c_destinatario = ?
    ORDER BY fecha DESC
");
$sql->execute([$numero_c, $numero_c]);
                        ...
                        <ul class="currency-list">
                            <?php if (!empty($historial)) : ?>
                                <?php foreach ($historial as $transferencia) : ?>
                                    <li><strong>Enviaste</strong>  $<?php echo number_format($transferencia['monto'], 2); ?></li>
                                    <li><strong>A</strong> <?php echo($transferencia['nombre_destinatario']); ?></li>
                                    <li><strong>Fecha:</strong> <?php echo $transferencia['fecha_formateada']; ?></li>
                                    <hr> <!-- Separador entre transferencias -->
                                <?php endforeach; ?>
                            <?php else : ?>
                                <li>No hay transferencias registradas.</li>
                            <?php endif; ?>
                        </ul>
                    </div>
